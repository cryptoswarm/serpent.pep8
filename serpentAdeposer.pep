...
; Par la description on veut dire la position initiale qui est composeé 
...
;
; Si une case du serpent se trouve à une case adjacente de la queue d'un autre
...
; L'allocation dynamique a été utiliseé dans ce programme. 
;
; Vous trouverez une explication exsaustive des noms des variables et methodes.                                                                   
;-----------------------------------------------------------------------------------------     
...
fini:            STA res1,d ; res1 = A;
                 ...
                 STA postem, d  
                 ;------------------------------;
                 ; Caccul de position de fin A5 ;
                 ;------------------------------;


posdeb:          LDX nbCol, i ; number of colom  =18 ;LDX  18,i  ;
debut1:          LDA 0, i
plus1:           ADDA 4, i
                 SUBX 1,i ; X--;
                 BRNE plus1
...
LdpoIn:          LDX matrix,d   ;ldx matrix,i     load la position initiale du serpent
                 ADDX serpPos,d 
                 STX serpPos,d
                 STX PdebutS, d ; la position du debut du serpent
                 
                 CALL Tail    ; Save the head which will be used as tail later

                 LDX matrix, d 
                 ADDX starPos, d              
                 STX starPos, d   ; la position A5

                 LDX matrix, d
                 ADDX endPos, d
                 STX endPos, d   ; la position R5
...
turnD:           LDA serpPos, d 
                 ...
                 CALL IfSDead   ; Appeler la methode qui verifie s'il yaura colision 
                                ; avec un autre serpent ou lui meme 
...
;if position debut et la position a A5 sont les memes, on verifie si cette case est rempli
                 LDA starPos, n
                 CPA empty, i
                 BREQ conti
                 LDX serpPos, d
...
KepLoad:         LDA '>', i
                 ...
                 LDX PdebutS, d  ; position a laquele le serpent a commence 
                 CPX starPos, d  ; ; position du serpent a la case A5
                 BRNE conti34 
...
;---------------------------------------------------------------------------
;----Enregistrer le debut de chaque serpent --------------------------------
;---------------------------------------------------------------------------
Tail:    LDA nbtail, d
         ADDA 1, i
         STA nbtail, d
...
Stail1: LDA serpPos, d
        STA tail1, d 
        RET0
         
Stail2: LDA serpPos, d
        STA tail2, d
        RET0
        
Stail3: LDA serpPos, d
        STA tail3, d
        RET0
        
Stail4: LDA serpPos, d
        STA tail4, d
         RET0        
        
Stail5: LDA serpPos, d
        STA tail5, d 
...
iloop3:   CPX     iSize,i
         ...
         BRGE    endend      ; Si on a terminé l'affichage du tableau avec position initiale
                             ; et parcours dedans on demande à  l'utilisateur de faire 
                             ; une nouvelle entree.
...
;---------------------------------------------------------------------------------------
;          Dans le cas où les specifications du serpent entrées sont correctes
;          mais menent à ce que le serpent se trouve a l'exterieur de l'espace de jeu
;                
;---------------------------------------------------------------------------------------
...
iloop4:   CPX     iSize,i
         ...
         BRGE    enderr      ; Si on a terminé l'affichage du tableau avec position initiale
                             ; et parcours dedans on demande à  l'utilisateur de faire 
...
carVert:      .EQUATE 0x0076   ; 
carHori:      .EQUATE 0x003E   
emptyO:       .EQUATE 0x006F    
tempCol:      .BLOCK 2
tempRow:      .BLOCK 2
bigMatx1:      .BLOCK 2
bigMatx2:      .BLOCK 2
...
CheKCar:   .BLOCK 2
charErr:   .BLOCK 2
specEr:   .BLOCK 2  ; char stocké les entress si on detecte erreur dans les specifiaction du serpent
...
PdebutS: .BLOCK 2 ; position du debut du serpent 
...
ix:      .BLOCK  2           ; #2d  reservÈ 2 octet ? ix initialisÈ ? 0 // rangee ou line 
 
jx:      .BLOCK  2           ; #2d  reservÈ 2 octet ? jx initialisÈ ? 0  // colonne 
range:   .WORD  1            ; nbr de rangee a imprimer 
rantemp:   .WORD  1  

temp:    .block  2           ;reservÈ 2 octet ? temp
...
MsgExt:  .ASCII  "Le serpent entré  depasse l'espace de jeu \x00"  
...
;------------------------------------------------------------------------------------------
;------  DExplication des noms utilisé dans le chois des variables et methode         ------
;--------------------------------------------------------------------------------------------
...
-----------------------------
 ...
 teteHaut: methode verifiant si la tete du serpent se dirige vers le haut d'une maniere 
             si on continue le jeu, le serpent serait à l'exterieur de l'espace de jeu.          

 Explication des variables:
---------------------------- 

 serpPos:  la position du serpent actuelle. cette position peut diminuer et augmenter
             en fonction du parcours du serpent. 
             par ex:    pour sauter d'une rangee à un autre, soit diminue 36 ou on ajoute 36.  